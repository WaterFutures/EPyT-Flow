...
from epyt_flow.simulation.scada import ScadaDataExport, ScadaDataNumpyExport, ScadaDataXlsxExport, \
...
def test_sensor_ordering():
    ...
    with ScenarioSimulator(scenario_config=hanoi_network_config) as sim:
        ...
        sde = ScadaDataExport(f_out=os.devnull)
        column_desc = sde.create_column_desc(scada_data)
        sensor_types = column_desc[:, 0]
        for i in range(4):
            assert sensor_types[i] == 'pressure'
        assert sensor_types[4] == 'flow'
